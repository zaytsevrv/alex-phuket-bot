    ...
    if 'user_data' in context.user_data:
        ...
        if (is_pregnant or has_young_children) and "Море" in user_choice:
            ...
            if is_pregnant or has_young_children:
                ...
                button_text += " (скорее всего пусто)"
...
def check_tour_restrictions(tour, user_data):
    ...
    if is_pregnant:
        ...
        elif "море" in category.lower() and "#можно_беременным" not in tags and "#можно_всем" not in tags:
            restrictions.append("🤰 Беременным не рекомендуются морские экскурсии")
    
    # Проверяем детей
    if children_ages:
        for age_months in children_ages:
            if age_months < 12:  # Дети до 1 года
                if "#дети_от_1_года" in tags or "море" in category.lower():
                    restrictions.append(f"👶 Детям до 1 года запрещены морские экскурсии")
