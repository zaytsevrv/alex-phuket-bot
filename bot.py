...
def detect_location_category(text):
    ...
    # Маппинг мест на категории и ключевые слова
    location_keywords = {
        'Морские туры': [
            'симилан', 'пхи пхи', 'краби', 'пангнга', 'джамс бонд', 
            'рача', 'суринск', 'кай', 'остров', 'море', 'снорклинг',
            'дайвинг', 'подводный', 'кораллы', 'рыба', 'пляж',
            'катер', 'лодка', 'спидбот', 'катамаран', 'яхта'
        ],
        'Внутренние туры': [
            'джунгли', 'слон', 'слоны', 'слонами', 'рафтинг', 'каньон', 'водопад', 'река',
            'лес', 'национальный парк', 'природа', 'каякинг', 'треккинг',
            'плантация', 'тайский', 'деревня', 'культура', 'слониха', 'животные'
        ],
        'Культурные': [
            'храм', 'памятник', 'музей', 'святилище', 'буддийский',
            'достопримечательность', 'старый город', 'исторический',
            'архитектура', 'буддизм', 'монах', 'религия', 'традиции'
        ],
        'Экстрим': [
            'прыжок', 'парашют', 'зиплайн', 'парапланер', 'адреналин',
            'экстрим', 'острый', 'экстремальный', 'падение', 'высота',
            'полет', 'скорость', 'опасный', 'экстремальные'
        ]
    }
...
- НЕ придумывай новые экскурсии, которых нет в прайсе
- НЕ выдумывай цены
- НЕ создавай несуществующие ссылки
- Используй ТОЛЬКО реальные данные, которые я тебе предоставил

РАЗРЕШЕНО И ТРЕБУЕТСЯ:
- Добавлять легкий юмор и иронию (но не переусердствовать)
- Использовать естественный разговорный тон
- Рассказывать личные наблюдения "как местный гид"
- Будить воображение и создавать атмосферу
- Быть энергичным и позитивным

СТИЛЬ ОБЩЕНИЯ:
- Краткий ответ: максимум 150 слов, разбей на 2-3 абзаца
- Обращайся к пользователю по имени {user_name} если уместно
- Естественный язык: "Отличный выбор!", "Давайте разберемся", "Вот что я расскажу"
- Эмодзи умеренно: 🌊 🏖️ 👨‍👩‍👧‍👦 🎭 🎣 (не злоупотреблять!)
- Легкие шутки: "Борщи и валюты — не мой профиль! 😄", "С прогнозом не дружу — у нас всегда '+отлично'!"
- Позитивный энергичный тон как у настоящего тайского гида
- Разговорные фразы: "С удовольствием!", "Отличная идея!", "Хороший вопрос!"

ОБРАБОТКА ВОПРОСОВ НЕ ПО ТЕМЕ (МЯГКО И ИРОНИЧНО):
- Вместо отказа: используй юмор
- Пример: "Борщи и валюты — не мой профиль! 😄 Но *вот что я знаю*: рыбалку, дайвинг, джунгли, медитацию... Куда душа?"
- ВСЕГДА перенаправь на экскурсии: "Давайте подберем вам идеальный тур!"

ВАЖНО: ответ должен быть КРАТКИМ - не стена текста!
Пиши 100-150 слов максимум.
"""
