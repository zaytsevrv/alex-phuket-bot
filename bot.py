...
from config import ADMIN_ID, BOT_STAGES, QUESTION_TYPES, ERROR_TYPES, EMOJI, pluralize_excursions, pluralize_hits
import json
...
# ==================== GIF АНИМАЦИИ ОТКЛЮЧЕНЫ ====================
# УБРАНО: GIF-анимации засоряли чат и не исчезали после ответа бота
...
def search_tours_by_keywords_hybrid(query):
    ...
    # Стоп-слова которые не помогают в поиске экскурсий
...
def make_category_keyboard(show_all=False):
    ...
    hit_categories = [
        "Море (Острова)",  # 38 экскурсий, самая большая
        "Суша (семейные)",  # Семейный туризм
        "Суша (обзорные)"   # 16 экскурсий, вторая по величине
    ]
...
def check_missing_points(user_data):
    ...
    if user_data['adults'] > 0 and user_data.get('children') is None:
        # Дети - это важный критерий безопасности (для морских экскурсий)
...
def get_smart_recommendations(user_data, current_category):
    ...
    if is_pregnant:
        ...
        if "Море" in current_category or "Яхты" in current_category:
            recommendations.append("🌊 *Море* — увы, запрещены все морские экскурсии")
...
def filter_tours_by_safety(tours, user_data):
    ...
    for tour in tours:
        ...
        if health_issues:
            ...
            # Укачивание - исключаем морские/скоростные экскурсии
...
def format_tour_card_compact(tour, index=None):
    ...
    # Формируем карточку с подчеркиванием названия
    if is_hit:
        card = f"{EMOJI['хит']} {num_prefix}<u>{name}</u>\n"
    else:
        card = f"{num_prefix}<u>{name}</u>\n"
    
    if desc_short:
        card += f"<i>{desc_short}</i>\n"
    
    card += f"{EMOJI['цена']} {price_adult}฿"
...
def format_tours_group(tours, title="", user_name="", show_tips=True):
    """
    Форматирует группу экскурсий с разделителем и подсказкой.
    Минималистично, без излишеств.
    ИСПРАВЛЕНО: добавлены разделители между экскурсиями
    """
    if not tours:
        return ""
    ...
    # Карточки туров с разделителями
    for i, tour in enumerate(tours, 1):
        if i > 1:
            result += "━" * 30 + "\n"  # Разделитель между экскурсиями
...
def make_tours_keyboard(tours, show_question_button=True, page=0, context=None):
    ...
    if page == 0:
        ...
        # ГИБРИДНАЯ КЛАВИАТУРА: собираем кнопки для размещения 2 в ряд
        tour_buttons = []
        for i, tour in enumerate(items_to_show):
            ...
            # Обрезаем слишком длинные названия для кнопок (меньше чем раньше)
            if len(display_name) > 28:
                display_name = display_name[:25] + "..."
            
            # Номер эмодзи вместо обычной цифры
            num_emoji = ['1️⃣', '2️⃣', '3️⃣'][i]
            
            # Используем ID тура для callback вместо индекса
            tour_id = tour.get('ID', '')
            tour_buttons.append(InlineKeyboardButton(f"{num_emoji} {display_name}", callback_data=f"tour_id_{tour_id}"))
        
        # РАЗМЕЩАЕМ КНОПКИ ПО 2 В РЯД
        if len(tour_buttons) >= 2:
            keyboard.append([tour_buttons[0], tour_buttons[1]])
            if len(tour_buttons) == 3:
                keyboard.append([tour_buttons[2]])
        elif len(tour_buttons) == 1:
            keyboard.append([tour_buttons[0]])
        
        # Если есть ещё ХИТы - показываем кнопку следующих ХИТов
        if total_hits > 3:
            remaining = total_hits - 3
            # ИСПРАВЛЕНО: правильное склонение "ХИТ"
            keyboard.append([InlineKeyboardButton(f"{EMOJI['меню']} Показать ещё ({pluralize_hits(remaining)})", callback_data=f"show_more_tours_1")])
...
async def handle_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПРОВЕРЯЕМ КНОПКУ "НОВЫЙ ПОИСК" - ПОЛНОСТЬЮ ОЧИЩАЕМ КОНТЕКСТ для нового клиента
    if user_choice == "🔄 Новый поиск":
        # Полностью очищаем все данные старого клиента
        context.user_data.clear()
...
• Возраст каждого ребенка (например: 5 лет, 3 года, менее 1 года)?
• Есть ли беременные в группе?
• Есть ли особые требования (здоровье, комфорт, бюджет)?

Например: "2 взрослых, ребенок 5 лет, не беременны, хотим комфорт"
"""
    ...
    # ПРОВЕРЯЕМ КНОПКИ РАЗВЁРТЫВАНИЯ КАТЕГОРИЙ (БЕЗ ТЕХНИЧЕСКИХ СООБЩЕНИЙ)
    if user_choice == "📂 Показать ещё категории":
        # ИСПРАВЛЕНО: убираем технические сообщения - просто показываем клавиатуру
        await update.message.reply_text(
            "Выбирайте:",
    ...
    if user_choice == "🔽 Скрыть категории":
        # ИСПРАВЛЕНО: убираем технические сообщения - просто скрываем клавиатуру
        await update.message.reply_text(
            "Основные категории:",
        ...
        if 'user_data' in context.user_data:
            ...
            if (is_pregnant or has_young_children) and "Море" in user_choice:
                ...
                if is_pregnant:
                    ...
                    response += "> 🔴 Запрещены **ВСЕ** морские экскурсии\n"
    ...
    if matching_tours:
        ...
        deepseek_comment = generate_deepseek_response(
            user_query=f"Пользователь спросил про: {user_choice}. Я нашел {len(matching_tours)} экскурсий по этому запросу. "
...
async def handle_tour_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'tour' in locals() and tour:
        ...
        if tour_index < len(ranked_tours):
            ...
            # Проверяем флаг readonly для морских экскурсий
...
✅ "По вашему запросу нашел 8 экскурсий со слонами. 
